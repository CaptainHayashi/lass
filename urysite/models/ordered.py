"""Arbitrarily ordered model support (from
...
class OrderedModel(models.Model):
    ...
    def save(self):
        if not self.id:
            try:
                self.order = (self.__class__.objects.all()
    ...
    def order_link(self):
        ...
        return '<a href="{0}">up</a> | <a href="{1}">down</a>'.format(
            url_up,
            url_down)
    ...
    def move_down(model_type_id, model_id):
        try:
            ModelClass = (ContentType.objects.get(id=model_type_id)
                          .model_class())
            ...
            lower_model.order, higher_model.order = (higher_model.order,
                                                     lower_model.order)
    ...
    def move_up(model_type_id, model_id):
        try:
            ...
            lower_model = ModelClass.objects.filter(
                order__lt=higher_model.order)[0]
